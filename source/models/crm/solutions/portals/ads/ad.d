module models.crm.solutions.portals.ads.ad;

@safe:
import uim.entities;

// Ad that is attached to one or more objects, including other notes.
  class DAPLAd : DOOPEntity {
  this() { super();
    this.attributes([
  ...
  auto webFile() {  
    if ("webFileId" in this.attributes) {
      if (collection && collection.tenant) {
        return collection.tenant[APLWebFile.entityClasses].findOne(["id": this.attributes["webFileId"].toString]);
      }}
    return null; }  
  unittest {
  ...
  auto webPage() {  
    if ("webPageId" in this.attributes) {
      if (collection && collection.tenant) {
        ...
        return collection.tenant[APLWebPage.entityClasses].findOne(["id": this.attributes["webPageId"].toString]);
      }}
    return null; }
  unittest {
    version(uim_entities) {
  ...
  auto webSite() {  
    if ("webSiteId" in this.attributes) {
      ...
      if (collection && collection.tenant) {
        return collection.tenant[APLWebSite.entityClasses].findOne(["id":id]);
      }
    }
    return null;
  }
}
auto APLAd() { return new DAPLAd; } 
auto APLAd(Json json) { return new DAPLAd(json); } 
