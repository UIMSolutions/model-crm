module models.crm.solutions.portals.external_identity;

@safe:
import uim.entities;

// ExternalIdentity that is attached to one or more objects, including other notes.
class DAPLExternalIdentity : DOOPEntity {
  this() { super();
    this.attributes([
...
auto APLExternalIdentity() { return new DAPLExternalIdentity; } 
auto APLExternalIdentity(Json json) { return new DAPLExternalIdentity(json); } 

unittest {
  version(uim_entities) {
    assert(APLExternalIdentity);
  
  auto entity = APLExternalIdentity;
