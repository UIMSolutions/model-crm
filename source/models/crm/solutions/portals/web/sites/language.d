module models.crm.solutions.portals.web.sites.language;

@safe:
import uim.entities;

// Languages supported and publishing status for the portal
  class DAPLWebSiteLanguage : DOOPEntity {
  this() { super();
    this.attributes([
  ...
  auto webSiteLanguage() {  
    if ("webSiteLanguageId" in this.attributes) 
      if (collection && collection.tenant) 
        return collection.tenant[APLWebSiteLanguage.entityClasses].findOne(["id": this.attributes["webSiteLanguageId"].toString]);
    return null;
  }
...
auto APLWebSiteLanguage() { return new DAPLWebSiteLanguage; } 
auto APLWebSiteLanguage(Json json) { return new DAPLWebSiteLanguage(json); } 

unittest {
  version(uim_entities) {
    assert(APLWebSiteLanguage);
  
  auto entity = APLWebSiteLanguage;
